...
# Summary 
Statistical models are considered simplification or approximation of reality [@burnham_model_2002]. How close to the target a given model is, or how it compares to competing models is always of interest in real-world applications Answers to such questions are mostly obtained via adequate goodness-of-fit (GOF) procedures. However, while such procedures alongside software implementations are readily available for the continuous outcome models, there are just a handful of open-source implementations available for categorical response models (CRMs).  
