...
idx <- c("mcfadden", "ugbagerth", "coxsnell", "nagelkerke",
         "mckelvey", "efron", "tjur")
...
xg2 <- vglm(fm, family = VGAM::cumulative(parallel = TRUE))
...
test_that("Rsquared function works properly",
          {
            ...
            expect_error(Rsquared(qs, measure="ugbagerth"))

            expect_vector(Rsquared(sm, measure = "ugbagerth")$R2)
            ...
            expect_vector(Rsquared(cm1, measure = "ugbagerth")$R2)
            expect_vector(Rsquared(cm2, measure = "mcfadden")$R2)
            expect_error(Rsquared(cm1, measure = "efron"))

            expect_vector(Rsquared(pm, measure = "ugbagerth")$R2)
            ...
            expect_equal(modtype(model=xg2, measure="ugbagerth", call.fn="Rsquared"), "vglm")
            ...
            expect_error(Rsquared(dd, measure = "ugbagerth"))
          })
